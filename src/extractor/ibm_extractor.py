    ...
    def extract(self, content: Dict[str, Any], source: Source) -> List[Claim]:
        ...
        for name in names:
            ...
            for role_info in roles:
                claim = Claim(
                    claim_text=f"{name} is {role_info['role']} at IBM",
                    claim_type="role",
                    value={
                        "person": name,
                        "role": role_info['role'],
                    ...
                    confidence_score=0.9,  # IBM press releases are highly credible for IBM-related info
                    sources=[source],
            ...
            for count in patent_counts:
                ...
                if re.search(context_pattern, full_text, re.IGNORECASE | re.DOTALL):
                    claim = Claim(
                        ...
                        confidence_score=0.9,  # IBM press releases are authoritative for patent counts
                        sources=[source],
